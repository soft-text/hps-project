...
// Configuration of the backend environment
type config struct {
	port int
	env  string
	db   struct {
...
type AppStatus struct {
	Status      string `json:"status"` // When you rendering as a JSON call this variable "status"
...
func main() {
	...
	flag.StringVar(&cfg.db.dsn, "dsn", "postgres://postgres:kurosak1@localhost/go_movies?sslmode=disable", "Postgres connection string")
	// just for development purposes will change later. Token generated from https://go.dev/play/p/s8KlqJIOWej with lib: "crypto/hmac", "crypto/sha256", "encoding/hex"
	...
	app := &application{
		config: cfg,
		logger: logger, // First logger related to  application logger in global variable, Second logger related to logger under main function
